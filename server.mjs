...
function normalizeQueryWords(value = "") {
  ...
  for (const token of words) {
    ...
    if (token.length > 4 && token.endsWith("es") && /(ches|shes|sses|xes|zes)$/.test(token)) {
...
function isPlainPlantQuery(search = "") {
  ...
  if (words.some((word) => traitLikeWords.has(word))) {
    return false;
  }
...
function buildSearchRankings(plants, query, parsed, region) {
...
function buildSearchComparison(plants, query, parsed, region) {
  const {
    ...
    strandbaseRanked,
    timings
  } = buildSearchRankings(plants, query, parsed, region);
...
function cueTokens(value = "", limit = 3) {
  const tokens = normalizeText(value)
    .split(/\s+/)
    .filter((token) => token && token.length > 1 && !CUE_STOPWORDS.has(token));
  return tokens.slice(0, limit);
}

function buildCueQuery(parts = [], maxTerms = 8) {
  const terms = [];

  for (const part of parts) {
    for (const token of cueTokens(part)) {
...
function toProofPlantKey(plant) {
  return normalizeText(plant?.id ?? plant?.constructStrand ?? plant?.name ?? "");
}

function proofTopMatches(items = [], limit = 3) {
  return items.slice(0, limit).map((item) => {
    const plant = item.plant ?? item;
    return {
      id: plant.id ?? plant.constructStrand ?? plant.name,
      name: plant.name,
      score: Number(item.score ?? plant.score ?? 0),
      plantType: plant.anchors?.plant_type ?? plant.plant_type ?? null
    };
  });
}

function rankForPlant(items = [], targetPlant) {
  const targetKey = toProofPlantKey(targetPlant);
  const index = items.findIndex((item) => toProofPlantKey(item) === targetKey);
  return index >= 0 ? index + 1 : null;
}

function summarizeActivatedProofStrands(strands = [], limit = 6) {
  return strands
    .filter((strand) => strand.kind !== "key_holder")
    .slice(0, limit)
    .map((strand) => ({
      kind: strand.kind,
      name: strand.name,
...
function humanizeCueLabel(label) {
  return label.replace(/_/g, " ");
}

function buildCueCandidates(plant) {
  const anchors = plant.anchors ?? {};
  const nameCue = String(plant.name ?? "").trim();
  const appearanceCue = buildCueQuery([
    anchors.primary_color,
    anchors.secondary_color,
    anchors.plant_type
  ]);
  const careCue = buildCueQuery([
    anchors.sunlight,
    anchors.moisture,
    anchors.growth_habit || anchors.plant_type
  ]);
  const signatureCue = buildCueQuery([
    anchors.fragrance,
    anchors.bloom_type,
    anchors.wildlife,
    anchors.plant_type
  ]);
  const groundCue = buildCueQuery([
    anchors.soil_type,
    anchors.moisture,
    anchors.growth_habit || anchors.plant_type
  ]);
  ...
  return [
    {
      id: "full_construct",
      label: "Full construct cue",
      query: nameCue,
      rationale: "Direct identity cue. This is the easiest construct reactivation test."
    },
    {
      id: "appearance_bundle",
      label: "Appearance bundle",
      query: appearanceCue,
      rationale: "Tests whether reusable appearance strands can recover the construct without naming it."
    },
    {
      id: "care_bundle",
      label: "Care bundle",
      query: careCue,
      rationale: "Tests whether care-related anchor strands point back to the same construct."
    },
    {
      id: "signature_bundle",
      label: "Signature bundle",
      query: signatureCue,
      rationale: "Tests whether a higher-order cue bundle can reactivate the construct from partial evidence."
    },
    {
      id: "ground_bundle",
      label: "Ground bundle",
      query: groundCue,
      rationale: "Tests whether soil and moisture structure still narrow the construct."
    }
  ].filter((cue, index, cues) => cue.query && cues.findIndex((item) => item.query === cue.query) === index);
}

function describeCueOutcome(tokenRank, strandRank) {
  const tokenHit = tokenRank !== null && tokenRank <= 3;
  const strandHit = strandRank !== null && strandRank <= 3;

  if (strandHit && !tokenHit) {
    return "Strandspace keeps the target in the top candidates from this partial cue while token overlap falls away.";
  }

  if (strandHit && tokenHit && strandRank < tokenRank) {
    return "Both methods recover the target, but Strandspace ranks it more confidently from the same cue bundle.";
  }

  if (strandHit && tokenHit) {
    return "Both methods recover the target. The strand trace makes the path to the construct explicit.";
  }

  if (strandRank !== null && tokenRank === null) {
    return "The target remains in Strandspace's candidate field even though the token baseline drops it.";
  }

  return "This cue is weaker. The construct is not yet stable from this signal alone.";
}

function sharedAnchorFacts(left, right) {
  const shared = [];
  const leftAnchors = left?.anchors ?? {};
  const rightAnchors = right?.anchors ?? {};

  for (const [name, value] of Object.entries(leftAnchors)) {
    const leftValue = normalizeText(value ?? "");
    const rightValue = normalizeText(rightAnchors[name] ?? "");
...
function sharedCompositeFacts(left, right) {
  const leftComposites = new Set((left?.composites ?? []).map((value) => normalizeText(value)));
  return (right?.composites ?? [])
    .filter((value) => leftComposites.has(normalizeText(value)))
    .map((value) => ({
      name: value,
      label: humanizeCueLabel(value)
    }));
}

function summarizeProofPlant(plant, region) {
  const anchors = plant?.anchors ?? {};
  return {
    id: plant?.id ?? plant?.constructStrand ?? plant?.name ?? "",
    name: plant?.name ?? "Plant",
    constructStrand: plant?.constructStrand ?? plant?.id ?? plant?.name ?? "",
    region,
    anchors: {
      plant_type: anchors.plant_type ?? null,
      primary_color: anchors.primary_color ?? null,
      sunlight: anchors.sunlight ?? null,
      soil_type: anchors.soil_type ?? null,
      moisture: anchors.moisture ?? null,
      fragrance: anchors.fragrance ?? null,
      height: anchors.height ?? null
    },
    composites: (plant?.composites ?? []).slice(0, 6)
  };
}

function buildProofRankingAnalysis(plants, query, parsed, targetPlant) {
  const targetKey = toProofPlantKey(targetPlant);
  ...
  const strandspaceStarted = performance.now();
  const strandbaseRanked = rankPlants(plants, parsed).map((item) => ({
    plant: item.plant ?? item,
    score: Number(item.score ?? 0)
  }));
  const strandspaceMs = Number((performance.now() - strandspaceStarted).toFixed(3));

  const tokenRankIndex = tokenizedRanked.findIndex((item) => toProofPlantKey(item.plant) === targetKey);
  const strandRankIndex = strandbaseRanked.findIndex((item) => toProofPlantKey(item.plant) === targetKey);

  return {
    tokenizedRanked,
    strandbaseRanked,
    tokenRank: tokenRankIndex >= 0 ? tokenRankIndex + 1 : null,
    strandRank: strandRankIndex >= 0 ? strandRankIndex + 1 : null,
    timings: {
      tokenizedMs,
      strandspaceMs,
      differenceMs: Number((tokenizedMs - strandspaceMs).toFixed(3))
...
function buildReuseMatches(targetPlant, plants, region) {
  return plants
    .filter((plant) => toProofPlantKey(plant) !== toProofPlantKey(targetPlant))
    .map((plant) => {
      const sharedAnchors = sharedAnchorFacts(targetPlant, plant);
      const sharedComposites = sharedCompositeFacts(targetPlant, plant);
      const score = sharedAnchors.length * 6 + sharedComposites.length * 12;
      return {
        score,
        plant: summarizeProofPlant(plant, region),
        sharedAnchors: sharedAnchors.slice(0, 5),
        sharedComposites: sharedComposites.slice(0, 4)
      };
    })
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || a.plant.name.localeCompare(b.plant.name))
...
function pickProofPlant(plants, requestedPlant = "") {
  const normalizedRequested = normalizeText(requestedPlant);
  if (normalizedRequested) {
    const exact = plants.find((plant) => {
      const keys = [plant.id, plant.constructStrand, plant.name, ...(plant.aliases ?? [])]
        .filter(Boolean)
        .map((value) => normalizeText(value));
      return keys.includes(normalizedRequested);
    });
  ...
  for (const exemplar of PROOF_EXEMPLARS) {
    const matched = plants.find((plant) => normalizeText(plant.name) === normalizeText(exemplar));
    if (matched) {
      return matched;
    }
  }
...
function buildProofPlantPool(plants, requestedPlant = "") {
  const pool = [];
  const requested = pickProofPlant(plants, requestedPlant);
  ...
  for (const exemplar of PROOF_EXEMPLARS) {
    const plant = plants.find((item) => normalizeText(item.name) === normalizeText(exemplar));
    if (plant && !pool.some((item) => toProofPlantKey(item) === toProofPlantKey(plant))) {
      pool.push(plant);
    }
  }

  if (pool.length === 0) {
    pool.push(...plants.slice(0, 10));
  }

  return pool.slice(0, 10);
}

function buildProofExemplars(plants, region) {
  const selected = [];

  for (const exemplar of PROOF_EXEMPLARS) {
    const plant = plants.find((item) => normalizeText(item.name) === normalizeText(exemplar));
    if (plant && !selected.some((item) => toProofPlantKey(item) === toProofPlantKey(plant))) {
      selected.push(plant);
    }
  }

  if (selected.length === 0) {
    selected.push(...plants.slice(0, 6));
  }

  return selected.slice(0, 6).map((plant) => summarizeProofPlant(plant, region));
}

function buildProofSummary(plant, cues = [], reuse = []) {
  const top3Hits = cues.filter((cue) => cue.strandspace.top3Hit).length;
  const tokenTop3Hits = cues.filter((cue) => cue.tokenized.top3Hit).length;

  return {
    selectedPlant: plant.name,
    anchorCount: Object.keys(plant.anchors ?? {}).length,
    compositeCount: (plant.composites ?? []).length,
    cueCount: cues.length,
    top3Hits,
    tokenTop3Hits,
    reuseMatches: reuse.length
  };
}

function buildTheoryClaims(summary) {
  return [
    {
      title: "Reusable anchors",
      body: `${summary.anchorCount} anchor strands are available for recombination instead of storing the construct as one flat label.`
    },
    {
      title: "Partial-cue recall",
      body: `Strandspace kept the selected construct in the top 3 for ${summary.top3Hits} of ${summary.cueCount} cue bundles.`
    },
    {
      title: "Shared structure",
      body: `${summary.reuseMatches} nearby constructs reuse anchor values or composite strands with the selected plant.`
    }
  ];
}

function buildProofReport(plants, region, requestedPlant = "") {
  const scopedPlants = buildProofPlantPool(plants, requestedPlant);
  const selectedPlant = pickProofPlant(scopedPlants, requestedPlant);
  if (!selectedPlant) {
    return null;
  }

  const cacheKey = `${region}:${toProofPlantKey(selectedPlant)}`;
  if (proofCache.has(cacheKey)) {
    return proofCache.get(cacheKey);
  }

  const cues = buildCueCandidates(selectedPlant).map((cue) => {
    const parsed = parseQuestion(cue.query);
    const rankings = buildProofRankingAnalysis(scopedPlants, cue.query, parsed, selectedPlant);
    const inspection = inspectStrands(cue.query, scopedPlants, { audience: "scientist" });
    const tokenRank = rankings.tokenRank;
    const strandRank = rankings.strandRank;
    return {
      ...cue,
      parsed: {
        intent: parsed.intent,
        attribute: parsed.attribute,
        plantPhrase: parsed.plantPhrase,
        trait: parsed.trait
      },
      tokenized: {
        rank: tokenRank,
        top3Hit: tokenRank !== null && tokenRank <= 3,
        topMatches: proofTopMatches(rankings.tokenizedRanked),
        totalMatches: rankings.tokenizedRanked.length
      },
      strandspace: {
        rank: strandRank,
        top3Hit: strandRank !== null && strandRank <= 3,
        topMatches: proofTopMatches(rankings.strandbaseRanked),
        totalMatches: rankings.strandbaseRanked.length,
        matchedPlant: inspection.matchedPlant?.name ?? rankings.strandbaseRanked[0]?.name ?? null,
        activatedStrands: summarizeActivatedProofStrands(inspection.activatedStrands ?? []),
        keyHolder: inspection.keyHolder?.relationReason ?? null,
        expressionField: inspection.expressionField ?? null
      },
      timings: rankings.timings,
      verdict: describeCueOutcome(tokenRank, strandRank)
    };
  });
  const reuse = buildReuseMatches(selectedPlant, scopedPlants, region);
  const summary = buildProofSummary(selectedPlant, cues, reuse);
  const report = {
    region,
    plant: summarizeProofPlant(selectedPlant, region),
    summary,
    theoryClaims: buildTheoryClaims(summary),
    cues,
    reuse,
    exemplars: buildProofExemplars(scopedPlants, region)
  };

  proofCache.set(cacheKey, report);
  return report;
}
...
function openGardenDatabase() {
  ...
  `);
  ensureSoundspaceTables(gardenDatabase);
  seedSoundspace(gardenDatabase);
  ensureSubjectspaceTables(gardenDatabase);
  seedSubjectspace(gardenDatabase);
...
function soundRecallNeedsExpansion(result = {}) {
  const parsed = result.parsed ?? {};
  const matched = result.matched ?? {};

  if (!matched || !result.ready) {
    return true;
  }

  if (parsed.sourceType && normalizeText(parsed.sourceType) !== normalizeText(matched.sourceType ?? "")) {
    return true;
  }

  if (parsed.eventType && normalizeText(parsed.eventType) !== normalizeText(matched.eventType ?? "")) {
    return true;
  }

  if (parsed.venueSize && normalizeText(parsed.venueSize) !== normalizeText(matched.venueSize ?? "")) {
    return true;
  }
...
async function handleApi(req, res, plants, flowers) {
  ...
  if (url.pathname === "/api/answer") {
    ...
    if (mode === "strandspace") {
      const strandspaceAnswer = await answerWithStrandspace(query, regionPlants, { audience });
      const llmAnswer = await answerWithLLM(query, regionPlants, { audience });
      sendJson(
        res,
        200,
        enrichResponseWithRegion(
          {
            ...strandspaceAnswer,
            llmTiming: {
              latencyMs: llmAnswer.latencyMs,
              cacheHit: llmAnswer.cacheHit,
              mode: llmAnswer.mode
            },
            tokenizedComparison
          },
  ...
  if (url.pathname === "/api/proof") {
    ...
    const proof = buildProofReport(regionPlants, region, requestedPlant);

    if (!proof) {
      sendJson(res, 404, {
        error: "No plants are available for the current proof scope."
      });
      return;
    }
  ...
  if (url.pathname === "/api/plants") {
    ...
    const searchParsed = parsed
      ? shouldTreatAsTraitList(parsed, regionMatches.map(({ plant }) => plant))
        ? broadTraitListParsed(parsed)
        : colorQuery && !parsed.attribute
          ? {
              ...parsed,
  ...
  if (url.pathname === "/api/soundspace" || url.pathname === "/api/soundspace/recall") {
    ...
    const result = recallSoundspace(openGardenDatabase(), question);

    sendJson(res, 200, {
      ok: true,
      ...result,
      libraryCount: listSoundConstructs(openGardenDatabase()).length
    });
    return;
  }
  ...
  if (url.pathname === "/api/subjectspace/library") {
    ...
    const constructs = listSubjectConstructs(openGardenDatabase(), subjectId);
  ...
  if (url.pathname === "/api/subjectspace" || url.pathname === "/api/subjectspace/recall") {
    ...
    const recall = recallSubjectSpace(openGardenDatabase(), {
      question,
      subjectId
    });
  ...
  if (url.pathname === "/api/soundspace/library") {
    ...
    const constructs = listSoundConstructs(openGardenDatabase());
    sendJson(res, 200, {
      ok: true,
      count: constructs.length,
      constructs
  ...
  if (url.pathname === "/api/soundspace/learn") {
    ...
    const saved = upsertSoundConstruct(openGardenDatabase(), {
      ...payload,
      provenance: {
    ...
    sendJson(res, 200, {
      ok: true,
      construct: saved,
      count: listSoundConstructs(openGardenDatabase()).length
    });
    return;
  }
  ...
  if (url.pathname === "/api/soundspace/answer") {
    ...
    if (!payload.forceGenerate && recalled.ready && recalled.matched && !soundRecallNeedsExpansion(recalled)) {
      sendJson(res, 200, {
        ok: true,
        source: "strandspace",
        question,
        answer: recalled.answer,
        construct: recalled.matched,
        recall: recalled
      });
      return;
    }
